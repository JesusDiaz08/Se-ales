...
% Last Modified by GUIDE v2.5 19-Jun-2018 16:24:12
...
% --- Executes on selection change in pp_interpolar.
function pp_interpolar_Callback(hObject, eventdata, handles)
% hObject    handle to pp_interpolar (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global myRecording
typeInter = pp_interpolar.Value;
auxP = size(myRecording);
x = (0:auxP-1);
Interp = str2double(get(handles.interpolar_texto,'String'));
Xq = (0:1/Interp:auxP);
...
sound(InterpD);
myRecording = InterpD;
% Hints: contents = cellstr(get(hObject,'String')) returns pp_interpolar contents as cell array
%        contents{get(hObject,'Value')} returns selected item from pp_interpolar


% --- Executes during object creation, after setting all properties.
function pp_interpolar_CreateFcn(hObject, eventdata, handles)
% hObject    handle to pp_interpolar (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
